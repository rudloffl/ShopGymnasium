...
class ShopEnv(gym.Env):
    def __init__(self,
        ...
        self.max_batchsize = 100
        
        # OBSERVATION SPACE - Dict is supported for observations
        self.observation_space = gym.spaces.Dict({
            # Stocks 
            "stockraw_used": gym.spaces.Box(low=0, high=1, shape=(1,), dtype=np.float32),
            "stockint_used": gym.spaces.Box(low=0, high=1, shape=(1,), dtype=np.float32),
            "stock2sell_used": gym.spaces.Box(low=0, high=1, shape=(1,), dtype=np.float32),

            # What is in progress and how many to do next
            'current_batch_remaining': gym.spaces.Box(low=0, high=1, 
                                                     shape=(self.product_count + self.machine_count,), 
                                                     dtype=np.float32),
            'next_batch': gym.spaces.Box(low=0, high=1, 
                                        shape=(self.product_count + self.machine_count,), 
                                        dtype=np.float32),
            ...
            "count_operators_busy": gym.spaces.Box(low=0, high=1, 
                                                   shape=(1,), dtype=np.float32),

            # Where is made what with the cycle time
            "prod_assignment": gym.spaces.Box(low=0, high=1, 
                                             shape=(self.product_count+ self.machine_count,), 
                                             dtype=np.float32),

            # Pending Reception
            "pending_reception": gym.spaces.Box(low=0, high=1, shape=(1,), dtype=np.float32),

            # Time of day
            "timeday": gym.spaces.Box(low=0, high=1, shape=(1,), dtype=np.float32)
        })
        ...
        self.episode_end = self.duration_max * self.hours_in_day  # End after 7 days
        self.salesrewards = 0
    ...
    def _make_simpy_env(self):
        ...
        self.stockraw_capacity = 100
        self.stockraw = simpy.Container(self.shopsim, capacity=self.stockraw_capacity, init=random.randint(1, self.stockraw_capacity-50))
        
        self.stockint_capacity = 50
        self.stockint = simpy.FilterStore(self.shopsim, capacity=self.stockint_capacity)
        self.stocksell_capacity = 50
        self.stocksell = simpy.FilterStore(self.shopsim, capacity=self.stocksell_capacity)
    ...
    def _parse_action(self, action_flat):
        ...
        Args:
            action_flat: Flattened numpy array from the policy (values between -1 and 1)
        ...
        action['current_batch'] = ((action_flat[start:end]+1)/2 * self.max_batchsize).astype(int).reshape(
            self.product_count, self.machine_count
        )
        
        # Extract force_current_batch (boolean)
        start, end = self.action_indices['force_current_batch']
        action['force_current_batch'] = (action_flat[start:end] > 0).astype(int).reshape(
            self.product_count, self.machine_count
        )
        
        # Extract next_batch (scale to 0-100)
        start, end = self.action_indices['next_batch']
        action['next_batch'] = ((action_flat[start:end]+1)/2 * self.max_batchsize).astype(int).reshape(
            self.product_count, self.machine_count
        )
        
        # Extract ranking_next (-1 to +1)
        start, end = self.action_indices['ranking_next']
        action['ranking_next'] = np.array((action_flat[start:end]+1)/2).reshape(
            self.product_count, self.machine_count
        )
        
        # Extract order_raw_prod (scale to 0-100)
        start, end = self.action_indices['order_raw_prod']
        action['order_raw_prod'] = int((action_flat[start:end][0]+1)/2 * self.stockraw_capacity)
    ...
    def _get_obs(self):
        ...
        return {
            ...
            "stockraw_used": np.array([self.stockraw.level / self.stockraw_capacity], dtype=np.float32),
            "stockint_used": np.array([len(self.stockint.items) / self.stockint_capacity], dtype=np.float32),
            "stock2sell_used": np.array([len(self.stocksell.items) / self.stocksell_capacity], dtype=np.float32),
            
            # These are already numpy arrays, ensure they're float32
            "current_batch_remaining": self.current_batch.flatten().astype(np.float32) / self.max_batchsize,
            "next_batch": self.next_batch.flatten().astype(np.float32) / self.max_batchsize,
            "ranking_next": self.ranking_next.flatten().astype(np.float32),
            
            # Convert operator counts to numpy arrays
            "count_operators_busy": np.array([self.operators.count / self.operatorcounts], dtype=np.float32),
            
            # Already numpy array, ensure float32
            "prod_assignment": np.array(self.prod_assignment / self.max_cycle_duration).flatten().astype(np.float32),
            
            # Convert pending reception to numpy array
            "pending_reception": np.array([self.pending_raw / self.stockraw_capacity], dtype=np.float32),
            
            # Convert time to numpy array
            "timeday": np.array([(self.shopsim.now % self.hours_in_day) / self.hours_in_day], dtype=np.float32)
        }
    ...
    def step(self, action_flat):
        ...
        # We order raw product Minimal order = 10
        if order_raw_prod > 10:
            if self.stockraw.capacity - self.stockraw.level >= order_raw_prod:
                self.shopsim.process(self.order_raw_product(order_raw_prod))
                self.order_log.append((self.shopsim.now, order_raw_prod))
            else:
                self.poormanagementpenality += order_raw_prod*10
                self.order_log.append((self.shopsim.now, -order_raw_prod))
    ...
    def steal_product_at_night(self, threshold_stock=50, day_hour=6, probability=.1):
    ...
    def make_products(self, machine_j=0, prod_i=0, patience=60):
        ...
        filterstore = False
        # print(self.shopsim.now, prod_i, machine_j)
        if prod_i == 1 and machine_j == 1:
            stockin = self.stockint
            filterstore = True
        
        # Determine stock destination
        stockout = self.stocksell
        if prod_i == 1 and machine_j == 0:
        ...
        with self.operators.request() as op_req, machine.request() as machine_req:
            ...
            while self.current_batch[prod_i, machine_j] != 0:
                ...
                if event.triggered:
                    ...
                    self.prod_trace.append((prod_start, self.shopsim.now, str(prod_i), machine_j))
                else:
                    # print('Giving up', prod_i, machine_j)
                    event.cancel()
                    break
        ...
        while True:
            if self.shopsim.now % freq == 0:
                ...
                for prod in allprods:
                    ...
                    solddict[prod] += 1
                for prod, qty in solddict.items():
                    self.sales_log.append((self.shopsim.now, prod, qty))
            yield self.shopsim.timeout(self.step_size)
